...
func (c *core) storeBacklog(msg *bft.Message, src bft.Validator) {
	logger := c.newLogger()
...
func (b *backlog) Push(msg *bft.Message) {
...
func (b *backlog) Pop(addr common.Address) (data *bft.Message, priority int64) {
	b.mu.Lock()
	defer b.mu.Unlock()
	...
	if _, ok := b.queue[addr]; !ok {
		...
		data = item.(*bft.Message)
		priority = p
		return
	}
}

var messagePriorityTable = map[bft.MsgType]int64{
	MsgTypeNewView:       1,
	MsgTypePrepare:       2,
...
func (b *backlog) toPriority(msgCode bft.MsgType, view *bft.View) int64 {
