...
func (c *core) sendPrepare() {
	...
	c.broadcast(&bft.Message{Code: msgTyp, Msg: payload})
	logger.Trace("sendPrepare", "prepare view", prepare.View, "proposal", prepare.Proposal.Hash())
}

func (c *core) handlePrepare(data *bft.Message, src bft.Validator) error {
	logger := c.newLogger()
...
func (c *core) createNewProposal() (bft.Proposal, error) {
	var req *bft.Request
...
func (c *core) extend(proposal bft.Proposal, highQC *bft.QuorumCert) error {
...
func (c *core) safeNode(proposal bft.Proposal, highQC *bft.QuorumCert) error {
	logger := c.newLogger()
